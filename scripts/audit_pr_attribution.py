...
Logic (kept in sync with contributor-check.yml):
  - scans direct first-parent commits since the merge base; imported upstream
    commits retain their upstream attribution without fork-specific mappings
...
def new_emails() -> list[str]:
    ...
    log = run("git", "log", f"{base}..HEAD", "--first-parent", "--format=%ae", "--no-merges", check=False)

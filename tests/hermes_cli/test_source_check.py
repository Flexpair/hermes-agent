...
def test_flexpair_origin_is_preserved_for_passive_source_checks(installation, origin):
    from hermes_cli.source_check import check_for_updates

    root, linked, home, base, head, responses, requests, git = installation
    git("remote", "set-url", "origin", origin)
    target = "c" * 40
    api_repository = origin.split("/", 3)[-1].removesuffix(".git") if origin.startswith("https://") else origin.split(":", 1)[-1].removesuffix(".git")

    responses[f"/repos/{api_repository}/commits/main"] = (200, target)
    responses[f"/repos/{api_repository}/compare/{head}...{target}"] = (
        200, {"ahead_by": 7, "commits": []}
    )
    ...
    assert requests[-2:] == [
        f"/repos/{api_repository}/commits/main",
        f"/repos/{api_repository}/compare/{head}...{target}",
    ]


def test_flexpair_main_tip_falls_back_to_its_origin_when_api_is_unavailable(installation):
    from hermes_cli import source_check

    root, _linked, _home, _base, _head, _responses, _requests, git = installation
    checkout = root / "flexpair-checkout"
    git("clone", str(root), str(checkout))
    expected = git("-C", str(root), "rev-parse", "refs/heads/main")

    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setattr(source_check, "_request", lambda *_a, **_k: (_ for _ in ()).throw(OSError("offline")))
        target, missing, failure = source_check._branch_tip(
            "flexpair/hermes-agent", "main", checkout, "git", "origin"
        )

    assert (target, missing, failure) == (expected, False, None)


def test_unknown_private_fork_does_not_treat_api_failure_as_deleted_main(installation):
    from hermes_cli import source_check
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setattr(source_check, "_request", lambda *_a, **_k: (_ for _ in ()).throw(OSError("offline")))
        target, missing, failure = source_check._branch_tip(
            "fixture/private-fork", "main", checkout, "git", "origin"
        )

    assert target is None
    assert missing is False
    assert failure == "api.github.com: offline"
...
def test_embedded_revision_keeps_https_ref_advertisement_recovery(installation, monkeypatch):
    ...
    assert probes[0][1]["env"]["GCM_INTERACTIVE"] == "Never"
...
@pytest.mark.parametrize("repository,heals,probe_repository", [
    ("NousResearch/hermes-agent", True, "NousResearch/hermes-agent"),
    ("fixture/fork", False, "fixture/fork"),
    ("Flexpair/hermes-agent", False, "Flexpair/hermes-agent"),
])
def test_official_ssh_healing_uses_public_https_without_retargeting_forks(
    installation, monkeypatch, repository, heals, probe_repository
):
    from hermes_cli.source_check import check_for_updates
    root, linked, home, base, head, responses, requests, git = installation
    ...
    if repository == "NousResearch/hermes-agent":
        ...
        git("config", f"url.{root.as_uri()}.insteadOf", f"https://github.com/{repository}.git")
    monkeypatch.setenv("GIT_SSH_COMMAND", "false")
    branch_file = home / "desktop-update.json"
    branch_file.write_text(json.dumps({"branch": "deleted"}))
    api_repository = probe_repository
    responses[f"/repos/{api_repository}/commits/main"] = (200, head)
    status = check_for_updates(install_root=linked, home=home, branch_config_path=branch_file)

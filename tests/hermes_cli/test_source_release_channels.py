...
def test_source_check_honors_transient_channel_without_rewriting_record(releases, monkeypatch, capsys):
    ...
    with patch("hermes_cli.main._resolve_update_branch", return_value="main"), patch.object(
...
def test_main_check_still_uses_branch_without_release_requests(releases, monkeypatch, capsys):
    set_install_channel("main", releases.root)
    monkeypatch.setattr(main, "PROJECT_ROOT", releases.root)
    update_cmd._cmd_update_check()

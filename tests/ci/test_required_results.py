...
def test_bootstrap_installer_skipped_release_fails_closed():
    verdict = evaluate_gate({"bootstrap-installer": {"result": "skipped"}}, release=True)

    assert verdict == {"ok": False, "failed": ["bootstrap-installer"], "allowed_skips": []}
...
def test_release_gate_requires_e2e_desktop_core_when_it_runs():
    verdict = evaluate_gate({"e2e-desktop-core": {"result": "skipped"}}, release=True)

    assert verdict == {"ok": False, "failed": ["e2e-desktop-core"], "allowed_skips": []}

...
def fetch_compare_branch(git_cmd: list[str], root: Path, branch: str, depth_args: list[str]):
    ...
    """
    origin_url = _uc()._get_origin_url(git_cmd, root)
    repository = _github_origin_repository(origin_url)
    supported_origin = repository == "flexpair/hermes-agent"
    is_fork = _uc()._is_fork(origin_url)
    if branch == "main" and is_fork and not supported_origin:
...
def _github_origin_repository(origin_url: str | None) -> str | None:
    from hermes_cli.source_releases import _GITHUB_ORIGIN

    match = _GITHUB_ORIGIN.fullmatch((origin_url or "").strip())
...
def compare_shallow_revisions(git_cmd: list[str], root: Path, compare_branch: str) -> int | None:
    """Count a shallow comparison against the repository that owns its fetched ref."""
    ...
    remote, _, branch = compare_branch.partition("/")
    if remote == "upstream" and branch == "main":
        repository = "NousResearch/hermes-agent"
    elif remote == "origin":
        repository = _github_origin_repository(
            _git(git_cmd, root, ["remote", "get-url", "origin"]).stdout.strip()
        )
    else:
        repository = None
    ...
    if repository:
        return _github_compare_behind(head_sha, target_sha, repository)
    return _github_compare_behind(head_sha, target_sha)
...
def report_shallow_verdict(git_cmd: list[str], root: Path, compare_branch: str) -> None:
    ...
    counted = compare_shallow_revisions(git_cmd, root, compare_branch)
    if counted == 0:

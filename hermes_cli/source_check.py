...
def _branch_tip(repository: str | None, branch: str, root: Path, git: str,
    ...
    if repository:
        ...
        # A 404 for an unknown origin can mean that the repository is private;
        # do not infer that its main branch was deleted. Supported public
        # distributions can safely retry through Git's configured transport.
        if branch == "main" and remote == "origin" and repository.lower() not in {
            OFFICIAL_REPOSITORY.lower(), "flexpair/hermes-agent",
        }:
...
def _check_branch(result: dict, co: _Checkout, selected_branch: str, *,
    ...
    if missing and selected_branch != "main":
        ...
        main_remote = remote if co.embedded else _branch_remote(co, "main")
        target, _, failure = _branch_tip(co.repository, "main", co.root, co.git, main_remote)
    if target is None:
...
def check_for_updates(*, install_root: Path | None = None, home: Path | None = None,
    ...
    unsupported = _unsupported_reason(stamp, root,
